	...
	SendMessage: (req: express.Request, res: express.Response) => {
		...
		try {
			getUserByToken(Auth, (err: any, result: any) => {
				...
				validationSendMessage(requestBody, (err: any, res_Validation: any) => {
					if (err) {
						res.status(400).json({
							code: 400,
							errors: {
						...
						if (res_Validation.phone.startsWith("0")) {
							...
							res_Validation.phone = "62" + res_Validation.phone + "@c.us";
						}
						if (!requestBody.media) {
							client
								...
								.then(
									(responseSuccess) => {
										res.status(200).json({
											code: 200,
											message: "Success Send Message To " + requestBody.phone,
									...
									(errSendMessage) => {
										res.status(400).json({
											code: 400,
											errors: {
												message: "Failed Send Message To " + requestBody.phone,
							...
							const media = MessageMedia.fromFilePath(res_Validation.media);
